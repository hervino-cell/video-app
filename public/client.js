...
function transportOptions(params) {
    return {
        ...
        iceTransportPolicy: 'all', // 'relay' bloquait tout si TURN non dispo
    };
}
...
function attachTrack(userId, consumer, kind) {
    ...
    video.muted = true; // FIX: toujours muted (l'audio passe par <audio> séparé)
    video.play()
        .then(() => {
            console.log('✅ Video playing:', userId);
        })
        .catch((err) => {
...
function setupSocketListeners() {
    ...
    socket.on('existing-producers', async (list) => {
        ...
        // Consommer en parallèle pour accélérer le chargement
        await Promise.all(list.map(({ producerId }) => consumeProducer(producerId)));
    });
